 ...
 */
void LineParamEstimator::estimate(std::vector<cv::Point2d *> &data, std::vector<double> &parameters)
{
	parameters.clear();
	if(data.size()<2)
		return;
 ...
 * Compute the line parameters  [n_x,n_y,a_x,a_y]
 */
void LineParamEstimator::leastSquaresEstimate(std::vector<cv::Point2d *> &data, std::vector<double> &parameters)
{
 ...
 */
bool LineParamEstimator::agree(std::vector<double> &parameters, cv::Point2d &data)
{
...
void LineParamEstimator::debugTest(std::ostream &out)
{
	...
	std::vector<cv::Point2d *> pointData;

	pointData.push_back(new cv::Point2d(7,7));
	pointData.push_back(new cv::Point2d(-1,-1));
	...
	pointData.push_back(new cv::Point2d(6,12));
	pointData.push_back(new cv::Point2d(6,6));
	lpEstimator.estimate(pointData,lineParameters);
	out<<"[n_x,n_y,a_x,a_y] [ "<<lineParameters[0]<<", "<<lineParameters[1]<<", ";
	out<<lineParameters[2]<<", "<<lineParameters[3]<<" ]"<<std::endl;
	lpEstimator.leastSquaresEstimate(pointData,lineParameters);
	...
	pointData.push_back(new cv::Point2d(7,9));
	pointData.push_back(new cv::Point2d(10,9));
	lpEstimator.estimate(pointData,lineParameters);
	out<<"[n_x,n_y,a_x,a_y] [ "<<lineParameters[0]<<", "<<lineParameters[1]<<", ";
	out<<lineParameters[2]<<", "<<lineParameters[3]<<" ]"<<std::endl;
	lpEstimator.leastSquaresEstimate(pointData,lineParameters);
